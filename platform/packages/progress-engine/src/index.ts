import type { EvidenceEvent, SkillState, SkillStateName } from '../../extension-contracts/src/types.ts';

export class ProgressEngine {
  calculate(profileId: string, skillId: string, evidence: EvidenceEvent[]): SkillState {
    const relevant = evidence.filter(
      (item) => item.profileId === profileId && item.skillId === skillId && item.result !== 'observed',
    );
    ...
    const weightedPositive = relevant.reduce((sum, e) => {
      ...
      return sum + independence * clamp(e.confidence, 0.25, 1) * (0.75 + clamp(e.difficulty, 0, 1) * 0.25);
    }, 0);
    const weightedNegative = failures * 0.55;
    const score =
      total === 0 ? 0 : clamp((weightedPositive - weightedNegative + total * 0.55) / (total * 1.55), 0, 1);
    ...
    if (total >= 5 && score >= 0.8 && independentSuccesses >= 3 && contexts.size >= 3) state = 'consolidated';
    return {
      profileId,
      skillId,
      state,
