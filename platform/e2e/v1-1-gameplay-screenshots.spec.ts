...
const rootValidationDir = path.resolve(process.cwd(), '..', '_validation', 'v1.1-10-10', 'games');
...
test.describe('V1.1 90 Gameplay Screenshots Capture', () => {
  ...
  for (const game of GAMES) {
    for (const vp of VIEWPORTS) {
      test(`${game.slug} - ${vp.name} (start, mid, success)`, async ({ page }) => {
